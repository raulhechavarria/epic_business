...
import time, os
from report.printscreen.ps_form import report_printscreen_list as printer_form

""""
This class was made to correct an error in the function to create table.
In parent class see the line 123 there was found an error, now we have fixed this error.
We have changed the original code: 'self.obj = render.rml(rml, self.title)', by this:
'self.obj = render.rml(rml, title=self.title)'. Besides we have to modify the create function for obtain
in print screen for one record, the same fields that appears in print screen for many records.
"""

class report_printscreen_list(printer_form):
    def __init__(self, name):
        printer_form.__init__(self, name)
